...
$(function () {
  ...
  let lastTime = $('.weekBox table tr').find('.time:last span').html();
  $('.weekBox table td').each(function () {
    $(this).find('.box:first').attr('data-time', firstTime);
    $(this).find('.box:last').attr('data-time', lastTime);
  });
  //
          ...
          $(this)
            ...
            .each(function (index) {
              ...
              $row
                ...
                .find('td')
                .eq(index)
                .prepend('<div class="tdTitle">' + thText + '</div>');
            });
        }
      });
  }
  ...
  var conBox = '.slideCon';
  $(conBox).parent().append($(conBox).clone());
  ...
  $('.loginBtn').keyup(function () {
    $('.font_size > a').removeClass('active').siblings('ul').stop(true, false).slideUp();
  });
...
$(function () {
  $('.inputBox select').change(function () {
...
if ($('body').find('.searchBlock').length > 0) {
  var dpMin, dpMax;
  dpMin = new AirDatepicker('#from', {
    onSelect({ date }) {
      dpMax.update({
        minDate: date,
      });
